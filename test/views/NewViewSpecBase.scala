...
trait NewViewSpecBase extends SpecBase {
  ...
  def assertRenderedByAttribute(doc: Document, attribute: String, optValue: Option[String]): Assertion =
    assert(
      optValue
        .fold(doc.getElementsByAttribute(attribute))(value => doc.getElementsByAttributeValue(attribute, value))
        .first() != null,
      "\n\nElement with attribute " + attribute + optValue
        .map(value => " and value " + value)
        .mkString + " was rendered on the page.\n"
    )
