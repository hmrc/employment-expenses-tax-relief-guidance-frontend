...
class ClaimingForControllerSpec extends ControllerSpecBase {
  ...
  def viewAsString(form: Form[_] = form) = claimingFor(frontendAppConfig, form)(fakeRequest, messages).toString
  ...
  "ClaimingFor Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(ClaimingForId.toString -> JsString(ClaimingFor.values.head.toString))
      val getRelevantData = new FakeDataRetrievalAction(Some(CacheMap(cacheMapId, validData)))

      val result = controller(getRelevantData).onPageLoad()(fakeRequest)

      contentAsString(result) mustBe viewAsString(form.fill(ClaimingFor.values.head))
    }
