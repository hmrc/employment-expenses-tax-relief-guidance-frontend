...
class $className$Controller @Inject()(
  ...
  def onPageLoad() = (getData andThen requireData) {
    implicit request =>
      ...
      Ok($className;format="decap"$(appConfig, preparedForm))
  }
    ...
    implicit request =>
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest($className;format="decap"$(appConfig, formWithErrors))),
        (value) =>
          dataCacheConnector.save[Int](request.sessionId, $className$Id.toString, value).map(cacheMap =>
            Redirect(navigator.nextPage($className$Id)(new UserAnswers(cacheMap))))
      )
  }
}
