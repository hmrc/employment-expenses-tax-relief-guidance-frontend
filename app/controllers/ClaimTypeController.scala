...
class ClaimTypeController @Inject()(
  ...
  def onPageLoad: Action[AnyContent] = getData {
    implicit request =>
      val preparedForm = request.userAnswers.flatMap(_.claimType) match {
        case None => form
  ...
  def onSubmit: Action[AnyContent] = getData.async {
    implicit request =>
      form.bindFromRequest().fold(
        ...
        value =>
          dataCacheConnector.save[Boolean](request.sessionId, ClaimTypeId, value).map(cacheMap =>
            Redirect(navigator.nextPage(ClaimTypeId)(new UserAnswers(cacheMap)))
          )
      )
  }
}
